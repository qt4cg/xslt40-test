<xsl:stylesheet version="4.0" 
   ...
   <!-- <xsl:import href="xsl-record-001.xsl"/>-->
    
    <xsl:output method="xml"/>
    
    <xsl:variable name="sales" as="map(*)*">
        <xsl:record product="'broiler'" quantity="20">
            <xsl:map-entry key="'store number'" select="1"/>
        </xsl:record>     
        <xsl:record product="'toaster'" quantity="100">
            <xsl:map-entry key="'store number'" select="2"/>
        </xsl:record>
        <xsl:record product="'toaster'" quantity="50">
            <xsl:map-entry key="'store number'" select="2"/>
        </xsl:record>
        <xsl:record product="'toaster'"  quantity="50">
            <xsl:map-entry key="'store number'" select="3"/>
        </xsl:record>
        <xsl:record product="'blender'" quantity="100">
            <xsl:map-entry key="'store number'" select="3"/>
        </xsl:record>
        <xsl:record product="'blender'" quantity="150">
            <xsl:map-entry key="'store number'" select="3"/>
        </xsl:record>
        <xsl:record product="'socks'" quantity="500">
            <xsl:map-entry key="'store number'" select="1"/>
        </xsl:record>
        <xsl:record product="'socks'" quantity="10">
            <xsl:map-entry key="'store number'" select="2"/>
        </xsl:record>        
        <xsl:record product="'shirt'" quantity="10">
            <xsl:map-entry key="'store number'" select="3"/>
        </xsl:record>        
    </xsl:variable>
    ...
    <xsl:variable name="stores" as="map(*)*">
        ...
        <xsl:record state="'MA'">
            <xsl:map-entry key="'store number'" select="4"/>
        </xsl:record>
    </xsl:variable>
  <!--  <xsl:variable name="sales" as="map(*)*">
      <xsl:map>
